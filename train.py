...
def parse_arg():
    ...
    parser.add_argument('-n_tree', type=int, default=12)
    parser.add_argument('-tree_depth', type=int, default=8)
...
def train(model, optim, db, opt):
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if not opt.jointly_training:
            ...
            with torch.no_grad():
                for batch_idx, (data, target) in enumerate(train_loader):
                    ...
                    target_int64 = target.type(torch.int64)
                    data = Variable(data)
                    # Get feats
                    feats = model.feature_layer(data)
                    feats = feats.view(feats.size()[0], -1)
                    feat_batches.append(feats)
                    target_batches.append(cls_onehot[target_int64])
                ...
                for tree in model.forest.trees:
                    ...
                    for _ in range(20):
                        ...
                        for mu, target in zip(mu_batches, target_batches):
                            ...
                            # print(prob, prob.shape) # [128, 10]
                            max_prob_row, index = torch.max(prob, axis=1)
                            # max_prob_row = np.array(max_prob_row)
                            max_prob.extend(max_prob_row)
                            # Variable to Tensor
                            pi = pi.data
                            prob = prob.data
                            mu = mu.data
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            loss = F.nll_loss(torch.log(output), target.type(torch.long))
            loss.backward()
            ...
            if batch_idx % opt.report_every == 0:
                ...
                # print('Probability is: ', ndf.Tree.cal_prob(mu, _mu, new_pi))
        # Eval
        model.eval()
        test_loss = 0
        correct = 0
        ...
        with torch.no_grad():
            for data, target in test_loader:
                if opt.cuda:
                    data, target = data.cuda(), target.cuda()
                data, target = Variable(data), Variable(target)
                # target_loss = torch.tensor(target, dtype=torch.long)
                output = model(data)
                test_loss += F.nll_loss(torch.log(output), target.type(torch.long),
            ...
            # print(prob, prob.shape)  # [96, 10]
            # print(len(max_prob))
            # print('Probability is: ', ndf.Tree.cal_prob(mu, _mu, new_pi))
            sorted_max_prob = np.sort(max_prob, axis=None)
    return sorted_max_prob
...
def main():
    ...
    sorted_max_prob = train(model, optim, db, opt)
    k = 5000
    threshold = sorted_max_prob[k]

    print(sorted_max_prob)
    print(threshold)
